...
describe('bug-03-missing-await regression', () => {
  test('waits for the insert to finish before resolving', async () => {
    ...
    const result = await saveUser(db, { id: 1 });

    expect(result).toEqual({ ok: true });
    expect(db.insert).toHaveBeenCalledWith('users', { id: 1 });
    expect(insertFinished).toBe(true);
  });

  test('rejects when the insert fails instead of leaking an unhandled rejection', async () => {
